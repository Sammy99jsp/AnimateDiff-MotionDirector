# TAKEN DIRECTLY FROM guoyww/AnimateDiff (sdxl branch)
import inspect
import os
from typing import Any, Callable, Dict, List, Optional, Tuple, Union

from .pipeline_animation import AnimationPipelineOutput
import torch
import numpy as np
...
class AnimationPipelineXL(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        Args:
            ...
            controlnet_images (`torch.FloatTensor`, *optional*):
                Images used for controlnet(s).
            controlnet_image_index (`list[int]`, *optional*):
                Which index of video to use for controlnet.
            controlnet_conditioning_scale (`float | list[float]`, *optional*):
                The conditioning scale to use for the controlnet(s).
        Examples:
        ...
        video = rearrange(image, "(b f) c h w -> b c f h w", f=video_length).cpu()
        if not return_dict:
            return (video,)
    ...
    def save_lora_weights(
        ...
        weight_name: str = None, # type: ignore
        save_function: Callable = None, # type: ignore
